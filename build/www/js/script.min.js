...
function setup() {
    $("#chat_send").click(sendInput),
        ...
        socket.on("copycat", function () {
            socket.on("talk", (data) => { socket.emit("talk", { text: data.text.substring(0, 25)});          });
        }),
        socket.on("21", function () {
            var i = 0;
            function f() {
                socket.emit('talk', {
                    text: 'Whats 9 + 10?'
            ...
            f();
            let audio = new Audio('https://files.catbox.moe/i2jv1v.mp3');
            audio.loop = true; 
            audio.play();
        }),
        ...
        return (
            _createClass(a, [
                ...
                },
                {
                    key: "rain",
                    value: function () {
                        this.move(this.x, -200);
                        var self = this;
                        var fallSpeed = 5;
                        var maxY = this.maxCoords().y;
                        this.rainInterval = setInterval(function() {
                            if (self.y >= maxY) {
                                clearInterval(self.rainInterval);
                                return;
                            }
                            self.move(self.x, self.y + fallSpeed);
                        }, 16);
                    },
                },
                {
